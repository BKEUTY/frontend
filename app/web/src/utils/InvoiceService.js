const generateInvoice = async (orderData, t) => {
    ...
    doc.setFont("Roboto", "normal");
    doc.setFontSize(10);
    const formattedOrderDate = orderData.orderDate ? new Date(orderData.orderDate).toLocaleDateString('vi-VN') : '---';
    doc.text(`${t('invoice_order_date')}: ${formattedOrderDate}`, 15, 57);
    if (orderData.estShippingDate) {
        const formattedEstDate = new Date(orderData.estShippingDate).toLocaleDateString('vi-VN');
    ...
    const tableData = orderData.items.map(item => {
        ...
        return [
            ...
            itemVoucherDiscount > 0 ? `-${itemVoucherDiscount.toLocaleString("vi-VN")}${t('unit_vnd')}` : `0${t('unit_vnd')}`,
            quantity,
